...
{
#ifdef LODEPNG_COMPILE_DECODER
  unsigned decompress(STLVcChar& out, const STLVcChar& in, const LodeZlib_DecompressSettings& settings = LodeZlib_defaultDecompressSettings);
#endif /*#ifdef LODEPNG_COMPILE_DECODER*/
#ifdef LODEPNG_COMPILE_ENCODER
  unsigned compress(STLVcChar& out, const STLVcChar& in, const LodeZlib_DeflateSettings& settings = LodeZlib_defaultDeflateSettings);
...
namespace LodePNG
{
  ...
  class Decoder : public LodePNG_Decoder
  {
    ...
    /*decoding functions*/
    void decode(STLVcChar& out, const unsigned char* in, size_t insize);
    void decode(STLVcChar& out, const STLVcChar& in);
    
    void inspect(const unsigned char* in, size_t size);
    void inspect(const STLVcChar& in);
  ...
  unsigned decode(STLVcChar& out, unsigned& w, unsigned& h, const unsigned char* in, unsigned size, unsigned colorType = 6, unsigned bitDepth = 8);
  unsigned decode(STLVcChar& out, unsigned& w, unsigned& h, const STLVcChar& in, unsigned colorType = 6, unsigned bitDepth = 8);
#ifdef LODEPNG_COMPILE_DISK
  unsigned decode(STLVcChar& out, unsigned& w, unsigned& h, const std::string& filename, unsigned colorType = 6, unsigned bitDepth = 8);
#endif /*LODEPNG_COMPILE_DISK*/
  ...
  class Encoder : public LodePNG_Encoder
  {
    ...
    void encode(STLVcChar& out, const unsigned char* image, unsigned w, unsigned h);
    void encode(STLVcChar& out, const STLVcChar& image, unsigned w, unsigned h);
  ...
  unsigned encode(STLVcChar& out, const unsigned char* in, unsigned w, unsigned h, unsigned colorType = 6, unsigned bitDepth = 8);
  unsigned encode(STLVcChar& out, const STLVcChar& in, unsigned w, unsigned h, unsigned colorType = 6, unsigned bitDepth = 8);
#ifdef LODEPNG_COMPILE_DISK
  ...
  unsigned encode(const std::string& filename, const STLVcChar& in, unsigned w, unsigned h, unsigned colorType = 6, unsigned bitDepth = 8);
#endif /*LODEPNG_COMPILE_DISK*/
...
#ifdef LODEPNG_COMPILE_DISK
  /*global functions allowing to load and save a file from/to harddisk*/
  void loadFile(STLVcChar& buffer, const std::string& filename);
  void saveFile(const STLVcChar& buffer, const std::string& filename);
#endif /*LODEPNG_COMPILE_DISK*/
...
simple functions to convert the files to/from buffers in memory.

This all makes LodePNG suitable for loading textures in games, raytracers,
intros, ..., or for loading images into programs that require them only for
simple usage. It's less suitable for full fledged image editors, loading PNGs
over network (since this decoder requires all the image data to be available
before the decoding can begin), life-critical systems, ... Even though it
contains a conformant decoder and encoder, it's still not a conformant editor,
...
LodePNG is written in C (ISO C90), and has a C++ wrapper around the C version.
The C++ wrapper adds RAII, the usage of std::vectors, and convenience functions.
...
it, and all the C code is made so that it compiles both with strict ISO C90 and C++.

To use the C++ version, you need to rename the source file to lodepng.cpp (instead
of lodepng.c), and compile this with a C++ compiler.

To use the C version, you need to rename the source file to lodepng.c (instead
of lodepng.cpp), and compile this with a C compiler. Optionally, you may remove
...
having to declare a struct or class).
...
unsigned LodePNG::decode(STLVcChar& out, unsigned& w, unsigned& h, const unsigned char* in, unsigned size);
unsigned LodePNG::decode(STLVcChar& out, unsigned& w, unsigned& h, const STLVcChar& in);
unsigned LodePNG::decode(STLVcChar& out, unsigned& w, unsigned& h, const std::string& filename);
...
unsigned LodePNG::encode(STLVcChar& out, const unsigned char* in, unsigned w, unsigned h);
unsigned LodePNG::encode(STLVcChar& out, const STLVcChar& in, unsigned w, unsigned h);
unsigned LodePNG::encode(const std::string& filename, const STLVcChar& in, unsigned w, unsigned h);
