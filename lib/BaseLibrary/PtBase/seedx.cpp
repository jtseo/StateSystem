...
#define EndianChange(dwS)                       \
    ( (ROTL((dwS),  8) & (UINT32)0x00ff00ff) |   \
      (ROTL((dwS), 24) & (UINT32)0xff00ff00) )


/************************ Block Encryption *************************/
void seed_Encrypt(BYTE *pbData, UINT32 *pdwRoundKey)
{
	UINT32 L0, L1, R0, R1, T0, T1, *K = pdwRoundKey;
...
{
  UINT32 A, B, C, D, T0, T1, *K=pdwRoundKey;

  A = ((UINT32 *)pbUserKey)[0];
  B = ((UINT32 *)pbUserKey)[1];
  C = ((UINT32 *)pbUserKey)[2];
  D = ((UINT32 *)pbUserKey)[3];
#ifndef BIG_ENDIAN
  A = EndianChange(A);
  B = EndianChange(B);
  C = EndianChange(C);
