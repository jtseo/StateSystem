...
#define ENDIAN_REVERSE_ULONG(dwS)	( (ROTL_ULONG((dwS),  8) & 0x00ff00ff)	\
	...
	#define BIG_B2D(B, D)		D = *(PULONG_PTR_SHA)(B)
	#define BIG_D2B(D, B)		*(PULONG_PTR_SHA)(B) = (ULONG)(D)
	#define LITTLE_B2D(B, D)	D = ENDIAN_REVERSE_ULONG(*(PULONG_PTR_SHA)(B))
	#define LITTLE_D2B(D, B)	*(PULONG_PTR_SHA)(B) = ENDIAN_REVERSE_ULONG(D)
#elif defined(LITTLE_ENDIAN)
	#define BIG_B2D(B, D)		D = ENDIAN_REVERSE_ULONG(*(PULONG_PTR_SHA)(B))
	#define BIG_D2B(D, B)		*(PULONG_PTR_SHA)(B) = ENDIAN_REVERSE_ULONG(D)
	#define LITTLE_B2D(B, D)	D = *(PULONG_PTR_SHA)(B)
	#define LITTLE_D2B(D, B)	*(PULONG_PTR_SHA)(B) = (ULONG)(D)
#else
...
// o 출력				: 
//*********************************************************************************************************************************
void SHA256_Transform(PULONG_PTR_SHA Message, PULONG_PTR_SHA ChainVar)
{
...
void SHA256_Process( OUT SHA256_INFO *Info, IN const BYTE *pszMessage, IN UINT uDataLen )
{
	...
	while (uDataLen >= SHA256_DIGEST_BLOCKLEN)
	{
		...
		SHA256_Transform((PULONG_PTR_SHA)Info->szBuffer, (PULONG_PTR_SHA)Info->uChainVar);
...
void SHA256_Close( OUT SHA256_INFO *Info, IN BYTE *pszDigest )
{
	...
	((PULONG_PTR_SHA)Info->szBuffer)[SHA256_DIGEST_BLOCKLEN / 4 - 2] = Info->uHighLength;
	((PULONG_PTR_SHA)Info->szBuffer)[SHA256_DIGEST_BLOCKLEN / 4 - 1] = Info->uLowLength;
