...
static Command_Entry* FindCommandEntry(SMTP_COMMAND command)
{
...
CSmtp::CSmtp()
{
	...
	if((RecvBuf = new char[BUFFER_SIZE_SMTP]) == NULL)
		throw ECSmtp(ECSmtp::LACK_OF_MEMORY);
	
	if((SendBuf = new char[BUFFER_SIZE_SMTP]) == NULL)
		throw ECSmtp(ECSmtp::LACK_OF_MEMORY);
	...
	try
	{
		...
		if(authenticate && IsKeywordSupported(RecvBuf, "AUTH") == true)
		{
			...
			else if(IsKeywordSupported(RecvBuf, "DIGEST-MD5") == true)
			{
				...
				//Create a cnonce
				char cnonce[17], nc[9];
				sprintf(cnonce, "%x", (unsigned int) time(NULL));
...
void CSmtp::ReceiveData(Command_Entry* pEntry)
{
	...
	{
		res = recv(hSocket,RecvBuf,BUFFER_SIZE_SMTP,0);
		if(res == SOCKET_ERROR)
		{
			FD_CLR(hSocket,&fdread);
...
void CSmtp::StartTls()
{
	...
	//strcpy_s(SendBuf, BUFFER_SIZE_SMTP, "STARTTLS\r\n");
	...
	while(!bFinish)
	{
		...
		if(FD_ISSET(hSocket,&fdread) || (read_blocked_on_write && FD_ISSET(hSocket,&fdwrite)) )
		{
			while(1)
			{
				...
				{
					if(offset + res > BUFFER_SIZE_SMTP - 1)
					{
						FD_ZERO(&fdread);
						FD_ZERO(&fdwrite);
						throw ECSmtp(ECSmtp::LACK_OF_MEMORY);
...
void CSmtp::ReceiveResponse(Command_Entry* pEntry)
{
	...
	//strcpy_s(RecvBuf, BUFFER_SIZE_SMTP, line.c_str());
